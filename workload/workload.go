...
func Setup(w Workload, numItemsArg int, numConcArg int, scp *gocb.Scope, coll *gocb.Collection) {
	workChan := make(chan DocType, numConcArg)
	shutdownChan := make(chan struct{}, numConcArg)
	var wg sync.WaitGroup

	wg.Add(numConcArg)
	for i := 0; i < numConcArg; i++ {
		go func() {
			for {
				select {
				case doc := <-workChan:
...
func Run(w Workload, numConc int, runTime time.Duration) {
